...
const BarChart = () => {
    ...
    if (view === "month") {
      ...
      return days.map((day) => ({
        ...
        revenue: monthData
          .filter(
            (item) =>
              Number(item.day) >= day && Number(item.day) < day + 3
          )
          .reduce((total, item) => total + item.revenue, 0),
      }));
    }
  ...
  return (
    <div className="w-full select-none rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm sm:p-6">
      ...
      <div className="h-80 w-full select-none outline-none">
