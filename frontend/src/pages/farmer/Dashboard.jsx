...
const orders = [
  { id: "ORD-2291", customer: "R. Ahmed", item: "Tomato · 12kg", status: "Confirmed" },
  { id: "ORD-2289", customer: "S. Karim", item: "Mango · 6kg", status: "Out for delivery" },
  { id: "ORD-2284", customer: "N. Islam", item: "Spinach · 20 bundles", status: "Delivered" },
];

const reviews = [
  { customer: "R. Ahmed", stars: 5, note: "Tomatoes arrived firm and fresh." },
  { customer: "F. Begum", stars: 5, note: "Consistent quality, always packed carefully." },
      ...
      <div className={`transition-all duration-200 ${showListings ? "scale-[0.995] blur-sm" : ""}`}>
        ...
        <section className="mx-auto max-w-7xl grid grid-cols-1 gap-10 px-6 py-14 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="flex flex-col justify-center">
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-muted">
              Green Fields Farm · Savar. Baseline prices move with stock and orders across FramFresh.
            </p>
        ...
        <section className="mx-auto max-w-7xl px-6 py-8">
          <div className="mb-5 flex items-center justify-between">
            ...
            <button type="button" onClick={() => window.location.href = "/farmer/Orders"} className="flex items-center gap-1 text-sm font-bold text-primary hover:text-primary-hover">
          ...
          <div className="overflow-hidden rounded-box border border-theme bg-base-200">
            ...
            {orders.map((order) => (
              <div key={order.id} className="grid grid-cols-1 gap-2 border-b border-theme-light px-5 py-4 last:border-b-0 md:grid-cols-[1fr_1.2fr_1.6fr_1fr] md:items-center md:gap-4">
                <span className="text-xs font-bold text-muted">{order.id}</span>
                <span className="text-sm">{order.customer}</span>
                <span className="text-sm">{order.item}</span>
                <span className="w-fit rounded-full px-3 py-1 text-xs font-bold md:ml-auto" style={{ color: statusColor[order.status], background: `${statusColor[order.status]}18` }}>
                  {order.status}
                </span>
              </div>
            ))}
        ...
        <section className="mx-auto max-w-7xl px-6 py-8">
          <div className="mb-5 flex items-center justify-between">
            ...
            <span className="flex items-center gap-2 rounded-full bg-primary-soft px-3 py-1.5 text-sm font-bold text-primary">
              <FiStar size={14} /> 4.8 aggregate
            </span>
          </div>
          
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {reviews.map((review) => (
              <div key={review.customer} className="rounded-box border border-theme bg-base-200 p-5 transition hover:-translate-y-1 hover:shadow-md">
                <div className="mb-3 flex gap-1 text-secondary">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <FiStar key={i} size={14} fill={i < review.stars ? "currentColor" : "none"} />
                  ))}
                </div>
                <p className="text-sm italic leading-6 text-muted">"{review.note}"</p>
                <div className="mt-4 text-xs font-bold text-muted-light">— {review.customer}</div>
              </div>
            ))}
          </div>
        </section>
